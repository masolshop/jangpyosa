...
1. ✅ **이 파일을 끝까지 읽으셨나요?** (최소 10분 소요)
2. ✅ **절대 금지 사항**을 숙지하셨나요? (아래 섹션)
3. ✅ **하이픈 정책**을 이해하셨나요? (전화번호/사업자번호)
4. ✅ **목업 데이터**와 **실제 데이터** 구분법을 아시나요?
5. ✅ **최근 커밋 10개**를 확인하셨나요? (`git log --oneline -10`)

**⚠️ 위 체크리스트를 모두 완료하지 않으면 데이터 손실 위험이 있습니다!**
...
# 5. DB 백업 상태 확인
ssh -i ~/.ssh/jangpyosa.pem ubuntu@43.201.0.129 'ls -lht /home/ubuntu/backups/jangpyosa/*.gz | head -3'
```
