...
- **최종 배포 시간**: 2026-02-22 00:00 KST
- **최신 커밋**: `10caa20` - 초대 링크 즉시 표시 완료 ✅
  - 초대받을 사람 정보 (이름, 핸드폰) 자동 입력 및 자동 채움
  - 비밀번호 자동 생성 (핸드폰 번호 뒤 8자리)
  - **초대 링크 접속 시 뒤로가기 없이 바로 회원가입 화면 표시** 🎯
  - 렌더링 조건 충돌 수정 (일반 회원가입 선택 화면 vs 초대 폼)
  - Nginx 캐시 정책 개선 (HTML은 no-cache, 정적 파일은 immutable)
