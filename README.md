...
- **최종 배포 시간**: 2026-02-22 03:00 KST
- **최신 커밋**: `eb823d5` - 월별 계산 로직 정밀화 완료 ✅
  - **중증 60시간 기준 수정**: 주 60시간 → **월 60시간** (올바른 기준 적용)
  - **중증 2명 인정**: 월 60시간 이상 근무 시 2명으로 인정 (부담금 계산)
  - **고용수준별 부담기초액 적용**: 장애인고용인원/의무고용인원 비율에 따라 차등
    - 0명 고용: 2,156,880원
    - 1/4 미만: 1,813,000원
    - 1/4~1/2: 1,554,000원
    - 1/2~3/4: 1,372,700원
    - 3/4 이상: 1,295,000원
  - **monthlyWorkHours 필드 사용**: 월 근로시간 기준으로 정확한 계산
- **이전 커밋**: `a9d2acf` - 직원 데이터 형식 표준화 (SEVERE/MILD, M/F)
  - 목업 직원 42명 재생성 (영문 코드 사용)
- **이전 커밋**: `c50e321` - 월 근로시간 기준 통계 시스템
