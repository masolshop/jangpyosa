...
- **최종 배포 시간**: 2026-02-22 02:15 KST
- **최신 커밋**: `20ff230` - 초대 코드 자동 관리 시스템 완료 ✅
  - **사용된 초대 코드 자동 삭제**: 팀원 가입 완료 시 초대 레코드 즉시 삭제
  - **초대 목록 필터링**: 사용되지 않은 초대만 조회 (isUsed: false)
  - **API 로직 개선**: auth.ts, team.ts 초대 관련 로직 최적화
  - **모든 기업/표준사업장 동일 적용**: BUYER, SUPPLIER 모든 계정에 적용
  - 초대 관리 효율성 향상 및 데이터베이스 정리 자동화
