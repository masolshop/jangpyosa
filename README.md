...
- **비용**: 1회당 40원
- **현재 모드**: `APICK_PROVIDER=real` (실제 API 사용)
- **API 엔드포인트**: `/api/apick/bizno/:bizNo`
- **테스트 완료**:
...
- **API 엔드포인트**: https://jangpyosa.com/api/ (Nginx 직접 프록시)
- **슈퍼어드민 로그인**: https://jangpyosa.com/admin/login
  - **초기 계정**: 전화번호 `01063529091` / 비밀번호 `01063529091`
...
- **리버스 프록시**: Nginx 1.18.0 (API 직접 프록시: /api/ → localhost:4000/)
- **SSL 인증서**: Let's Encrypt (자동 갱신, 2026-05-22까지 유효)
- **데이터베이스**: SQLite (프로덕션 환경, dev.db)
- **DNS**: Cafe24 (직접 연결, Cloudflare 미사용)
- **보안**: HTTPS/TLS 1.2+1.3, HSTS, HTTP→HTTPS 자동 리다이렉트
- **최종 배포 시간**: 2026-02-22 02:45 KST
- **최신 커밋**: `61b4c8d` - 직원 근로시간 표시 개선 완료 ✅
  - **2026년 최저시급 기준 적용**: 10,320원
  - **월 근로시간 기준 급여 계산**: 월 60~209시간 랜덤
  - **급여 = 월 근로시간 × 10,320원** (정확한 계산)
  - **직원 카드 UI 개선**: 근로시간 월 XX시간 | 월 XXX원 (시급 표시)
  - **목업 직원 51명 재생성**: buyer01(15명), buyer03(11명), buyer05(14명), supplier01(11명)
  - 최소 급여 619,200원 (월 60시간), 최대 2,156,880원 (월 209시간)
- **이전 커밋**: `20ff230` - 초대 코드 자동 관리 시스템 완료 ✅
- **이전 커밋**: `e8d31cc` - 회사공지업무방 권한 확장 완료 ✅
  - **모든 기업 유형 및 표준사업장 팀원 접근 가능** (BUYER, SUPPLIER, SUPER_ADMIN)
  - **시각장애인을 위한 음성 읽기 기능 추가** (Web Speech API)
- **이전 커밋**: `b8f5b4e` - 목업 직원 데이터 생성 완료 ✅
  - **총 55명 장애인 직원 목업 데이터 등록** (buyer01, buyer03, buyer05, supplier01)
  - 중증/경증 랜덤 배치, 월 60시간 이상 근무

### 🔐 테스트 계정 (2026-02-21 최신)

**로그인 시스템 재설정 완료** ✅
- 데이터베이스 초기화 및 Prisma 스키마 동기화
- 10개 지사, 전체 회원 계층 구조 재생성
- 모든 계정 로그인 테스트 완료

**1️⃣ 슈퍼어드민 (전화번호 로그인)**
- `01063529091` / `01063529091` - SUPER_ADMIN

**2️⃣ 매니저 (전화번호 로그인)**
- `010-9876-5432` / `agent1234` - AGENT (서울남부지사, 추천코드: AGENT001)

**3️⃣ 표준사업장 (ID 로그인)**
- `supplier01` / `test1234` - SUPPLIER
  - 회사: 행복한표준사업장
  - 사업자번호: 1234567890
...
**민간기업 (3.1%)**
- `buyer01` / `test1234` - 민간기업1 (사업자번호: 1111122222, 담당: 김과장)
  - 장애인직원: 홍길동 1명 등록
- `buyer02` / `test1234` - 민간기업2 (사업자번호: 2222233333, 담당: 이대리)

**공공기관 (3.8%)**
- `buyer03` / `test1234` - 공공기관1 (사업자번호: 3333344444, 담당: 최부장)
  - 장애인직원: 박영희 1명 등록
- `buyer04` / `test1234` - 공공기관2 (사업자번호: 4444455555, 담당: 조차장)

**국가/지자체/교육청 (3.8% + 감면)**
- `buyer05` / `test1234` - 교육청1 (사업자번호: 5555566666, 담당: 한교육사)
  - 장애인직원: 이철수 1명 등록
- `buyer06` / `test1234` - 지자체1 (사업자번호: 6666677777, 담당: 신주무관)

**5️⃣ 장애인 직원 (전화번호 로그인)**
- `010-1001-0001` / `employee123` - 홍길동 (민간기업1 소속)
  - 장애유형: 지체, 중증, 월급 250만원
- `010-1001-0002` / `employee123` - 박영희 (공공기관1 소속)
  - 장애유형: 시각, 중증, 월급 280만원
...
- ✅ 직원 로그인 시 회사명 표시 문제 해결
  - 백엔드 응답 구조 차이 대응: `user.company.name` (기업) / `user.companyName` (직원)
  - Sidebar에서 회사명 자동 감지 및 표시
- ✅ 직원 로그인 시 "직원 메뉴" 섹션 자동 표시
  - EMPLOYEE 역할 확인 후 "⏰ 출퇴근 관리" 메뉴 노출
  - 페이지 이동 시 useEffect로 동적 업데이트
- ✅ 로그인 상태에 따른 실시간 메뉴 전환
  - 기업 담당자: 기업 메뉴 활성화, 직원 메뉴 숨김
  - 직원: 직원 메뉴 표시, 기업 메뉴 비활성화 (🔒)
  - 로그아웃: 초기 상태로 복원
- ✅ 10명 목업 직원 계정 생성 완료
  - 민간기업1 (buyer01, 사업자번호 1111122222) 소속
  - 핸드폰: 010-1001-0001 ~ 010-1001-0010
  - 비밀번호: employee123
  - 중증 6명, 경증 4명 / 여성 5명 / 60~80시간 근무

**Phase 2.3 완료: 회원가입 폼 레이아웃 정렬 수정 (2026-02-21)**
- ✅ `globals.css`에 폼 입력 필드 스타일 추가
  - label, input, select, textarea, button 기본 스타일 정의
  - focus, disabled 상태 스타일 추가
  - 드롭다운 화살표 아이콘 스타일링
  - 입력 필드 간 적절한 여백 및 정렬
- ✅ 회원가입 페이지 레이아웃 정렬 개선
  - 기업 유형 선택 버튼 (민간기업, 공공기관, 국가/지자체/교육청)
  - 사업자등록번호 입력 + APICK 인증 버튼
  - 로그인 ID, 비밀번호 필드
  - 담당자 정보 (성함, 직책, 이메일, 핸드폰)
  - 추천인 매니저 핸드폰 번호
  - 개인정보 동의 체크박스
- ✅ APICK 실제 API 연동 완료
  - `/api/apick/bizno/:bizNo` 엔드포인트로 변경
  - PM2 환경변수 설정: `APICK_PROVIDER=real`
  - 실제 사업자번호 조회 테스트 완료: 266-81-01215 → 주식회사 페마연 ✅

**Phase 2.1 완료: 주민번호 앞자리 추가 및 3중 매칭 시스템 강화**
- ✅ `/dashboard/employees` 폼에 주민번호 앞자리 입력 필드 추가
  - 필수 입력 항목 (직원 회원가입 인증용)
  - 6자리 숫자 형식 제한
  - 생년월일과 일치 필수
- ✅ 엑셀 샘플 및 업로드에 주민번호 앞자리 컬럼 추가
  - 필수 항목 검증: 성명, 주민번호앞자리, 장애유형, 입사일
  - 샘플 데이터: 850315, 900720, 881130
- ✅ 직원 목록에 주민번호 앞자리 표시
  - 보안을 위해 마스킹 처리: 850*** (앞 3자리만 표시)
- ✅ 직원 회원가입 3중 매칭 검증 강화
  - 사업자등록번호 → 기업 확인
  - 이름 → 직원 매칭
  - 주민번호 앞자리 → 직원 인증
...
## 🔑 초기 계정 정보 (2026-02-21 AWS 배포 기준)

### 슈퍼어드민 로그인 (핸드폰 번호 + 비밀번호)

| 역할 | 핸드폰 번호 | 비밀번호 | 기능 |
|------|------------|----------|------|
| Super Admin | 010-6352-9091 | 01063529091 | 슈퍼어드민 대시보드, 회원 관리, 견적 관리 |
| Super Admin (구) | 010-1234-5678 | admin1234 | (구버전, 사용 가능) |

**로그인 URL**: https://jangpyosa.com/admin/login

### 기업 계정 로그인 (핸드폰 번호 + 비밀번호)

| 역할 | 핸드폰 번호 | 비밀번호 | 기업명 | 기업 유형 |
|------|------------|----------|--------|-----------|
| 공공기관 | 010-7777-8888 | test1234 | 한국공공기관 | PUBLIC_INSTITUTION (3.8%) |
| 국가/지자체 | 010-9999-0000 | test1234 | 서울특별시청 | GOVERNMENT (3.8%+감면) |

### 매니저 계정 로그인 (핸드폰 번호 + 비밀번호)

| 역할 | 핸드폰 번호 | 비밀번호 | 추천코드 | 소속 지사 |
|------|------------|----------|---------|----------|
| 매니저 | 010-9876-5432 | agent1234 | AGENT001 | 서울남부지사 |
| 매니저 | 010-8765-4321 | agent1234 | AGENT002 | 서울남부지사 |

**참고사항:**
- 모든 로그인은 핸드폰 번호로 가능 (하이픈 자동 제거)
- 기업 계정은 username이 없으므로 핸드폰 번호로 로그인
- 비밀번호는 평문 그대로 입력 (서버에서 bcrypt로 검증)
...
# 4) 직원 10명 확인 (김민수, 박영희, 이철수, ...)
# 5) 월별 고용 관리 페이지 이동
URL: /dashboard/monthly

# 6) 상시근로자 1000명 입력 후 자동 계산 확인
- 의무고용: 31명
- 인정인원: 16명 (중증 6명×2 + 경증 4명×1)
- 부족인원: 15명
- 월 부담금: 20,179,350원
```
...
#### 3. 직원 회원가입 테스트 (3중 매칭 검증)
```bash
# 전제조건: 기업 담당자가 /dashboard/employees에서 직원 등록 필요

# 1) 직원 회원가입 페이지 접속
URL: /employee/signup

# 2) 입력 정보
이름: 김민수 (반드시 기업 등록 이름과 일치)
핸드폰: 010-9999-9999 (새 번호)
비밀번호: newpass123 (8자 이상)
비밀번호 확인: newpass123
...
# 3) 회원가입 버튼 클릭
→ 3중 매칭 검증:
  ① 사업자번호 1111122222 → 민간기업1 존재 확인
  ② 이름 "김민수" → 해당 기업의 직원 중 이름 매칭
  ③ 주민번호앞자리 850315 → 해당 직원의 주민번호앞자리 일치
  ④ 재직 중(resignDate가 null) 확인

# 4) 성공 시 자동 로그인 및 리다이렉트
→ /employee/attendance
```
