...
- **최종 배포 시간**: 2026-02-22 00:12 KST
- **최신 커밋**: `6b770e2` - 초대 회원가입 폼 개선 완료 ✨
  - 이메일 필수 항목으로 변경
  - 직함 입력 필드 추가
  - **아이디(영문+숫자) 필드 추가** - 사용자가 직접 설정
  - **비밀번호/비밀번호 확인 필드 추가** - 사용자가 직접 설정 (8자 이상)
  - 자동 비밀번호 생성 제거 (보안 강화)
  - 아이디·비밀번호 찾기 링크 추가 (/forgot-password)
  - 담당자 정보 입력 간소화 (직함만 선택 입력)
  - 백엔드 API 및 스키마 업데이트 (username, 비밀번호 검증 강화)
