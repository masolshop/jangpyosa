...
- **최종 배포 시간**: 2026-02-22 01:36 KST
- **최신 커밋**: `e8d31cc` - 회사공지업무방 권한 확장 완료 ✅
  - **모든 기업 유형 및 표준사업장 팀원 접근 가능** (BUYER, SUPPLIER, SUPER_ADMIN)
  - **민간기업, 공공기관, 국가지자체교육청, 표준사업장 모두 접근 가능**
  - **팀원 계정(초대받은 계정)도 회사 공지사항 관리 가능** - getUserCompany 적용
  - 제목 변경: "회사 공지사항 관리" → "회사공지업무방"
  - **시각장애인을 위한 음성 읽기 기능 추가** (Web Speech API)
  - 자동 음성 읽기 토글 스위치 (localStorage 저장)
  - 각 공지마다 음성 재생 버튼 (🔊 듣기/⏸️ 중지)
  - 한국어 음성 지원 (ko-KR), 접근성 개선
  - 표준사업장 권한 확장 및 팀원 접근 정상화 완료
