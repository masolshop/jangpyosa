...
export default function EmployeesPage() {
  ...
  function downloadExcelSample() {
    ...
    const sampleNotes = [
      ...
      [],
      ["[주민번호 앞자리]"],
      ["- 주민등록번호 앞 6자리를 입력합니다 (예: 850315 = 1985년 3월 15일생)"],
      ["- 직원 회원가입 시 본인 인증에 사용됩니다"],
      ["- 장려금 계산 시 생년월일로 자동 변환됩니다"],
      ["- ⚠️ 생년월일 컬럼은 별도로 추가하지 마세요 (주민번호로 자동 계산)"],
      [],
      ["[중증여부]"],
                ...
                {(tab === "active" ? activeEmployees : resignedEmployees).map((emp) => {
                  ...
                  >
                    {isEditing ? (
                      // 📝 수정 모드
                      <form onSubmit={handleSubmit}>
                        <h3 style={{ marginTop: 0, marginBottom: 20, color: "#3b82f6" }}>
                          ✏️ {emp.name} 정보 수정
                        </h3>
                        <div
                          style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}
