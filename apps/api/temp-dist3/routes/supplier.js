...
// 공급사 프로필 조회
router.get('/profile', requireAuth, requireRole('SUPPLIER'), async (req, res) => {
    try {
...
// 공급사 프로필 업데이트
router.put('/profile', requireAuth, requireRole('SUPPLIER'), async (req, res) => {
    try {
...
// 이미지 업로드 (최대 5개)
router.post('/profile/images', requireAuth, requireRole('SUPPLIER'), upload.array('images', 5), async (req, res) => {
    try {
        const user = await prisma.user.findUnique({
            where: { id: req.user.id },
            include: { company: { include: { supplierProfile: true } } },
...
// 이미지 삭제
router.delete('/profile/images/:imageIndex', requireAuth, requireRole('SUPPLIER'), async (req, res) => {
    try {
