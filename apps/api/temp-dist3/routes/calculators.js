...
function calcStandardBenefitV2(b) {
    // ---- 1) 무상지원금(항목별 + 적격만 합산) ----
    const items = [
    ...
    return {
        grant: {
            ...
            expertSupport,
            grantTotal,
            rule: "무상지원금: (적격 항목 합계와 공단산정금액 중 작은 금액)×75%, 단 신규장애인고용 1인당 4천만원(최대 10억) 한도 적용"
        },
        tax: {
            annualCap,
            taxes10y,
            yearly,
            taxReductionTotal,
            rule: "세액감면: 1~3년 100%, 4~5년 50%, 6~10년 30% (연도별 한도: 1억 + 장애인상시근로자×2천만원)"
        },
        totalBenefit,
        disclaimer: "본 계산은 추정치이며 실제 지원·감면은 요건 충족 및 심사/승인 결과에 따라 달라질 수 있습니다."
    };
}
// 부담금 계산기 (게스트도 사용 가능)
r.post("/levy", async (req, res) => {
    try {
        ...
        res.json({
            ...
            disabledCount: body.disabledCount,
            ...out,
            note: "본 결과는 설정된 부담기초액 기반의 추정치입니다. 실제 산정·신청은 한국장애인고용공단/부담금 시스템 기준을 확인하세요.",
        });
    }
    catch (error) {
...
r.post("/linkage", async (req, res) => {
    try {
        ...
        res.json({
            ...
            contractAmount: body.contractAmount,
            ...out,
            rule: "감면 총액은 부담금의 90% 이내이며, 도급액의 50%를 초과할 수 없습니다(상한 자동 적용).",
            note: "연계고용 부담금 감면제도는 장애인표준사업장 등과 도급계약을 체결해 납품받는 경우, 해당 사업장에서 종사한 장애인을 고용한 것으로 간주해 부담금을 감면하는 제도입니다.",
        });
    }
    catch (error) {
...
r.post("/standard-workplace-benefit-v2/report.pdf", async (req, res) => {
    try {
        ...
        doc.moveDown(0.4);
        out.tax.yearly.forEach((y) => {
            doc.text(`${y.yearIndex}년차 | 산출세액 ${n(y.tax)}원 | 감면율 ${Math.round(y.rate * 100)}% | 감면(캡) ${n(y.cappedReduction)}원`);
        });
        doc.moveDown(0.4);
        ...
        doc.moveDown(0.8);
        doc.fontSize(10).fillColor("#666").text("※ 실제 지원·감면은 요건 충족, 서류, 승인 및 심사 결과에 따라 달라질 수 있습니다.\n" +
