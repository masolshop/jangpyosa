...
r.post("/login", async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({ userId: user.id, role: user.role }, config.jwtSecret, { expiresIn: "7d" });
        const refreshToken = jwt.sign({ userId: user.id }, config.jwtRefreshSecret, { expiresIn: "30d" });
        return res.json({
            accessToken,
            refreshToken,
            user: {
...
r.post("/update-profile", async (req, res) => {
    try {
        ...
        if (body.managerName)
            updateData.managerName = body.managerName;
        if (body.managerTitle !== undefined)
            updateData.managerTitle = body.managerTitle;
        if (body.managerEmail)
            updateData.managerEmail = body.managerEmail;
...
r.post("/login/employee", async (req, res) => {
    try {
        ...
        const accessToken = jwt.sign({ userId: user.id, role: user.role, employeeId: user.employeeId }, config.jwtSecret, { expiresIn: "7d" });
        const refreshToken = jwt.sign({ userId: user.id }, config.jwtRefreshSecret, { expiresIn: "30d" });
        return res.json({
            accessToken,
            refreshToken,
...
r.post("/signup-invited", async (req, res) => {
    try {
        ...
        // 7. JWT 토큰 생성
        const accessToken = jwt.sign({ userId: newUser.id, role: newUser.role }, config.jwtSecret, { expiresIn: "7d" });
        const refreshToken = jwt.sign({ userId: newUser.id }, config.jwtRefreshSecret, { expiresIn: "30d" });
