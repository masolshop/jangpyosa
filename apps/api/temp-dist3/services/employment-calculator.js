 ...
 * 의무고용률 조회 (회사 유형별)
 */
function getQuotaRate(companyType) {
 ...
 * 고용수준 = 장애인고용인원 / 의무고용인원
 */
export function getLevyBaseAmount2026(disabledCount, obligatedCount) {
    if (obligatedCount === 0)
        return LEVY_BASE_AMOUNTS_2026.LEVEL_0;
    if (disabledCount === 0)
        return LEVY_BASE_AMOUNTS_2026.LEVEL_0;
    const employmentRate = disabledCount / obligatedCount;
    if (employmentRate >= 0.75)
        return LEVY_BASE_AMOUNTS_2026.LEVEL_3_4_OVER; // 3/4 이상
    if (employmentRate >= 0.5)
        return LEVY_BASE_AMOUNTS_2026.LEVEL_1_2_TO_3_4; // 1/2 ~ 3/4 미만
    if (employmentRate >= 0.25)
        return LEVY_BASE_AMOUNTS_2026.LEVEL_1_4_TO_1_2; // 1/4 ~ 1/2 미만
 ...
 * 근로시간에 따른 비율
 */
function getWorkHoursRate(hoursPerWeek) {
    if (hoursPerWeek >= 40)
        return 1.0; // 100%
    if (hoursPerWeek >= 30)
        return 0.75; // 75%
 ...
 * 특정 월의 장애인 직원 필터링
 */
export function getEmployeesForMonth(employees, year, month) {
 ...
 */
export function calculateEmployeeMonthly(employee, year, month) {
    ...
    // ========================================
    // 기본 단가 (성별 구분)
    const baseIncentive = getBaseIncentiveRate(employee.severity, employee.gender);
    // 월임금액의 60% 상한 적용
    ...
    const isEligibleForIncentive = monthsWorked <= maxSupportPeriod &&
 ...
 * 월별 전체 계산
 */
export function calculateMonthlyData(employees, totalEmployeeCount, year, month, companyType = "PRIVATE") {
    // 해당 월 근무 중인 직원 필터링
    const activeEmployees = getEmployeesForMonth(employees, year, month);
    // 각 직원별 계산
    const details = activeEmployees.map((emp) => calculateEmployeeMonthly(emp, year, month));
    // ========================================
    // 집계
    // ========================================
    // 장애인 수
    ...
    // 인정 장애인 수 (중증 2배 포함)
    const recognizedCount = details.reduce((sum, d) => sum + d.levyRecognizedCount, 0);
    // 의무고용인원
 ...
 * 연간 전체 계산 (12개월)
 */
export function calculateYearlyData(employees, monthlyEmployeeCounts, year, companyType = "PRIVATE") {
    const results = [];
    for (let month = 1; month <= 12; month++) {
        const totalEmployeeCount = monthlyEmployeeCounts[month] || 0;
        const monthlyResult = calculateMonthlyData(employees, totalEmployeeCount, year, month, companyType);
